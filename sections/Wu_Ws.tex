В данной главе рассматривается исключительно устойчивый лист медленного многообразия. Поэтому для упрощения обозначений индексы, указывающие на принадлежность к устойчивому или неустойчивому листу, будут опущены: все функции и константы подразумеваются заданными на устойчивом листе, если не оговорено иное.

Также для удобства сместим положение равновесия \textit{седло-центр} системы в точку $0$, при этом все обозначения оставим прежними:
$$\Lambda \rightarrow \Lambda, \quad \quad \lambda \rightarrow \lambda - \lambda_+(\hat x_0,0) = \lambda - \pi, \quad \quad x \rightarrow x - \hat x_0, \quad \quad y \rightarrow y.$$

В новых переменных уравнения имеют вид:
\begingroup
\footnotesize
\begin{equation}
    \left\{
    \begin{aligned}
        \dot \Lambda &= - U \sin \lambda + V \cos \lambda \equiv g^\Lambda(\Lambda,\lambda,x,y),,
        &\quad
        \dot \lambda &= \alpha \Lambda \equiv g^\lambda(\Lambda,\lambda,x,y), \\[1.5ex]
        \dot x &= -\varepsilon \big( 2Fy-\frac{\partial U}{\partial y} \cos \lambda - \frac{\partial V}{\partial y} \sin \lambda \big) \equiv g^x(\Lambda,\lambda,x,y),
        &\quad
        \dot y &= \varepsilon \big( 2F(x+\hat x_0)+G -\frac{\partial U}{\partial x} \cos \lambda - \frac{\partial V}{\partial x} \sin \lambda \big) \equiv g^y(\Lambda,\lambda,x,y).
    \end{aligned}
    \right.
    \label{fulltn}
\end{equation}
\endgroup

$$U(x,y) = U_0 + x(2C \hat x_0+D)+C(x^2-y^2),\quad \quad V(x,y) = 2Cxy+y(2C \hat x_0+D),$$
$$U_0 = U(\hat x_0^+, 0).$$

Заметим, что система (\ref{fulltn}) является гамильтоновой относительно симплектической формы (\ref{sform}) с гамильтонианом:
\begin{equation}
H = \frac{\alpha \Lambda^2}{2} - U(x,y)\cos{\lambda} - V(x,y)\sin{\lambda} + F \left((x+\hat x_0)^2+y^2 \right) +  G x. 
\label{H}
\end{equation}
