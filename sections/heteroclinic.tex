Так как радиус сходимости рядов для периодических решений имеет порядок $\mathcal{O}(\varepsilon)$ (т.е. $|h| \le R(\varepsilon) = \mathcal{O}(\varepsilon)$), введем масштабированный параметр $\mu \equiv h/\varepsilon$:
$$|\mu| \le \frac{R(\varepsilon)}{\varepsilon} = \mathcal{O}(1)$$

Тогда систему (\ref{final_avg_ham}) можно рассматривать как малое возмущение исходного гамильтониана $H_{old}(\v z)$:
\begin{equation}
\langle H \rangle_0(\v z) = H_{old}(\v z) + \varepsilon^2 \mu^2 H_1(\v z) + \mathcal{O}(\varepsilon^4),
\label{cut_ham}
\end{equation}
...
\end{multline*}
Здесь $[ \cdot ]_0$ обозначает взятие свободного коэффициента в разложении по степеням $\varepsilon$.
