...
На каждом из листов медленного многообразия такая система имеет единственное положение равновесия:
$$
y_0^{\pm} = 0,\quad
...
где
$$A = 2 U(x_0^{\pm}, y_0^{\pm})(\pm F - C), \quad
B = (2Cx_0^{\pm}+e_J D)(\pm F - C), \quad
\delta = -(2Cx_0^{\pm}+e_J D)^2.$$
Если ввести новые переменные:
$$\xi  = \left( X+\frac{B}{A} \right) \sqrt{\frac{-A}{F^2-C^2-\frac{B^2}{A}}} \equiv \omega \left( X+\frac{B}{A} \right) \in \mathbb{R}, \quad \quad 
\eta = Y \sqrt{\frac{A+\delta}{F^2-C^2-\frac{B^2}{A}}} \equiv \sigma Y \in \mathbb{R},$$
...
\end{equation}
где
$$P(s) \equiv s^2 \left( \frac{1}{\omega^2} + \frac{1}{\sigma^2} \right) + s \frac{-2B }{\omega A} + \left( \frac{B^2}{A^2}-\frac{1}{\sigma^2} \right),$$
$$Q(s) \equiv  -\frac{s^2}{\omega} \left( \frac{1}{\omega^2}+\frac{1}{\sigma^2} \right) + s \frac{2B}{A} \left( \frac{1}{\sigma^2}+\frac{1}{\omega^2} \right) - \frac{1}{\omega} \left( \frac{1}{\sigma^2} + \frac{B^2}{A^2} \right).$$

С другой стороны, подставляя (\ref{eight}) в первое уравнение системы (\ref{slow_sys}), получаем:
$$x' = \frac{y \Big(2(C^2-F^2)(x^2+y^2) + 2x(2Cx_0+e_J D)(C \mp F) + (2Cx_0+e_J D)^2 + 2U(x_0, y_0)(-C \mp F)  \Big)}{F(x^2+y^2) \pm U(x_0, y_0) \pm x(2Cx_0+e_J D)}.$$

Тогда в терминах переменной $f$ получаем
\begin{equation}
\label{der_x_2}
x' = \frac{\sh f}{\sigma P(\ch f)} \frac{(A+\delta)Q(\ch f)}{S(\ch f)},
\end{equation}
где
$$S(s) \equiv \pm U(x_0,y_0) P(s) \pm \left( \frac{s}{\omega} - \frac{B}{A}\right) (2Cx_0+e_J D) + F.$$
Сравнивая выражения (\ref{der_x_1}) и (\ref{der_x_2}), получаем дифференциальное уравнение на $f$:
\begin{equation}
\label{der_f}
f' = \frac{A+\delta}{\sigma} \frac{P(\ch f)}{S(\ch f)}.
\end{equation}
Заметим, что $P, Q, S$ являются полиномами второго порядка переменной $\ch f$.
Интегрируя уравнение (\ref{der_f}), получаем:
\begin{equation}
\tau -\tau_0 = \frac{\sigma}{A+\delta} \int_{f_0}^f \frac{S(\ch f)}{P(\ch f)} df.
\label{f_tau}
\end{equation}

Таким образом на обоих листах $M_{0,+}, M_{0,-}$ медленная система (\ref{slow_sys}) имеет неустойчивое положение равновесия $(x,y) = (x_0^{\pm},y_0^{\pm})$. Это положение равновесия порождает двумерную сепаратрису, заполненную решениями вида:
